...
/**
 * Fetch all contests from Kontests API with timeout
 * @returns {Promise<Array>} Array of raw contest objects
 */
export async function fetchKontestsContests() {
  try {
    ...
    // Filter to include only major platforms we want
    const majorPlatforms = ['codeforces', 'leetcode', 'codechef', 'atcoder', 'topcoder', 'hackerrank', 'hackerearth'];
    
    const filtered = data.filter(contest => {
      const site = contest.site?.toLowerCase();
      return majorPlatforms.some(platform => site?.includes(platform));
    });
...
export async function fetchKontestsByPlatform(platform) {
  try {
    const allContests = await fetchKontestsContests();
...
export async function getKontestsPlatforms() {
  try {
    const contests = await fetchKontestsContests();
